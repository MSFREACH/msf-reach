...
export default (config, db, logger) => ({
   ...
   */
	...
	createEvent: (body) => new Promise((resolve, reject) => {
		...
		logger.debug(query, values);
		db.oneOrNone(query, values).timeout(config.PGTIMEOUT)
			.then((data) => resolve({ id: data.id, status: data.status, type:body.type, created: body.created, reportkey:data.report_key, metadata:body.metadata, the_geom:data.the_geom }))
			.catch((err) => reject(err));
	}),
	...
	updateEvent: (id, body) => new Promise((resolve, reject) => {
		...
		logger.debug(query, values);
		db.oneOrNone(query, values).timeout(config.PGTIMEOUT)
			.then((data) => resolve({ id: String(id), status: body.status, type:data.type, created: data.created, reportkey:data.report_key, metadata:data.metadata, the_geom:data.the_geom }))
			.catch((err) => reject(err));
	})
});
